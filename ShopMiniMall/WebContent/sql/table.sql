...
(
  ...
  phone1 varchar2(3) not null, --폰번호1
  phone2 varchar2(4) not null, --폰번호2
  phone3 varchar2(4) not null, --폰번호3
  email1 varchar2(20) not null, --이메일1
  email2 varchar2(20) not null --이메일2
);
...
insert into goods 
  ...
  create table cart
  (  num NUMBER(6) PRIMARY KEY, -- 장바구니 번호(기본키)
     userid VARCHAR2(10), -- 회원 아이디(외래키)
     gCode varchar2(20) not null, -- 상품 코드(외래키)
     gName varchar2(50) not null, -- 상품 이름
     gPrice NUMBER(6) not null, -- 상품 가격
     gSize CHAR(1) not null, -- 상품 사이즈
     gColor VARCHAR2(10) not null, -- 상품 색깔
     gAmount NUMBER(2) not null, -- 상품 개수
  ...
  add CONSTRAINT cart_gCode_fk FOREIGN KEY(gCode)
   REFERENCES goods(gCode) ON DELETE CASCADE; 
    ...
    create sequence cart_seq; -- 장바구니 번호를 자동으로 증가시키기 위해 시퀀스 생성
  ...
  (  num NUMBER(6) PRIMARY KEY, -- 주문 번호
     userid VARCHAR2(10), -- 회원 아이디
     ...
     gAmount NUMBER(2) not null, -- 상품 개수
     gImage varchar2(20) not null, -- 상품 이미지
     orderName VARCHAR2(10) NOT NULL, -- 주문 이름
     post VARCHAR2(5) NOT NULL, -- 우편번호
     addr1 VARCHAR2(500) NOT NULL, -- 주소1
     addr2 VARCHAR2(500) NOT NULL, -- 주소2
     phone VARCHAR2(11) NOT NULL, -- 폰 번호
     payMethod VARCHAR2(10) NOT NULL, -- 결제수단
     orderDay DATE  DEFAULT SYSDATE -- 주문 날짜 (오늘)
  );   
